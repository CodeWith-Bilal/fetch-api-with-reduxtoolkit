    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm ${getStatusColor(apiStates.users.status)}`}>
            <span className="mr-2">{getStatusIcon(apiStates.users.status)}</span>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm ${getStatusColor(apiStates.posts.status)}`}>
            <span className="mr-2">{getStatusIcon(apiStates.posts.status)}</span>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm ${getStatusColor(apiStates.weather.status)}`}>
            <span className="mr-2">{getStatusIcon(apiStates.weather.status)}</span>
            {apiStates.weather.status}
          </div>
          {apiStates.weather.error && (
            <p className="text-red-600 text-sm mt-2">{apiStates.weather.error}</p>
          )}
          <div className="mt-2 text-sm text-gray-600">
            Mock API (Demo data)
          </div>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm ${getStatusColor(apiStates.github.status)}`}>
            <span className="mr-2">{getStatusIcon(apiStates.github.status)}</span>
            {apiStates.github.status}
          </div>
          {apiStates.github.error && (
            <p className="text-red-600 text-sm mt-2">{apiStates.github.error}</p>
          )}
          <div className="mt-2 text-sm text-gray-600">
      ...
      <div className="mt-8 bg-gray-50 rounded-lg p-4">
        <h3 className="text-lg font-semibold mb-3">Redux Toolkit Features Demonstrated</h3>
